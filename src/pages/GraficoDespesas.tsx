...
const GraficoDespesas = () => {
    ...
    const fetchDespesas = async () => {
      try {
        const response = await http.get(
          "https://back-end-financeiro.onrender.com/despesas/W7AABqLwfVXz0YfmFgTbiC66Vmn2"
        ); // Rota do backend
  ...
  const totalPorCategoria = despesas.reduce((acc, despesa) => {
    ...
    acc[categoria] += despesa.valor; // soma o valor a categoria correspondente
    return acc;
  }, {});

  // Dados do gráfico
  const formattedLabels = Object.keys(totalPorCategoria).map(
    (label) => label.charAt(0).toUpperCase() + label.slice(1)
  );
  const data = {
    labels: formattedLabels, // Categorias formatadas
    datasets: [
      {
        label: " ",
        ...
        borderWidth: 1,
        hoverBackgroundColor: palette.map((color) =>
          color.replace("0.5", "1.0")
          ...
          generateLabels: (chart: any) => {
            return chart.data.labels.map((label: any, index: number) => {
              ...
              return {
                ...
                strokeStyle: palette[index % palette.length].replace(
                  "0.5",
                  "1"
                ),
                lineWidth: 1,
    ...
    <div style={{ height: "500px", width: "80%", margin: "0 auto" }}>
      ...
      {tipoGrafico === "Bar" ? (
        ...
        <PolarArea
          ref={chartRef}
          data={data}
          options={{ ...options, scales: {} as any } as any}
        /> // Remove as escalas no Polar Area
      )}
    </div>
  );
};
